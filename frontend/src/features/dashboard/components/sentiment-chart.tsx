...
type CombinedDataPoint = {
    ...
    low: number | null;
    close: number | null;
    openClose: [number, number] | null;
    highLow: [number, number] | null;
    isUp: boolean | null;
    score: number | null;
    sma: number | null;
    count: number;
...
type SentimentTooltipProps = {
    active?: boolean;
    payload?: Array<{ payload: CombinedDataPoint }>;
    label?: string;
};
...
export const SentimentChart = ({ ticker, startDate, endDate }: SentimentChartProps) => {
        ...
        const loadData = async () => {
            ...
            try {
                const [m, s] = await Promise.all([
                    dashboardService.getMarketData(ticker, startDate, endDate),
                    dashboardService.getSentimentHistory(ticker, startDate, endDate)
                ]);
                setMarketData(m);
                setSentimentData(s);
            } catch (error) {
    ...
    const CustomTooltip = ({ active, payload, label }: SentimentTooltipProps) => {
                    ...
                    <div className="h-[70%] w-full relative">
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                            <ComposedChart
                                ...
                                onMouseDown={handleMouseDown as unknown as never}
                            >
