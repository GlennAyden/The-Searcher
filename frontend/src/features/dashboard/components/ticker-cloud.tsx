...
import { neobdmApi, type SignalItem } from '@/services/api/neobdm';
...
export const TickerCloud = () => {
    ...
    useEffect(() => {
        const fetchSignals = async () => {
            setLoading(true);
            try {
                const response = await neobdmApi.getNeoBDMHotList();
                ...
                const uniqueSignals = Array.from(
                    new Map(cleanedSignals.map(item => [item.symbol, item])).values()
                );
