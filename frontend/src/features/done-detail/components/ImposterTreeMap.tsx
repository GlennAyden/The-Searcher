...
type ImposterTreeNode = {
    ...
    possible_count?: number;
    fill?: string;
    children?: ImposterTreeNode[];
};

type ImposterTreemapContentProps = {
    x?: number;
    y?: number;
    width?: number;
    height?: number;
...
type ImposterTreemapTooltipProps = {
    active?: boolean;
    payload?: Array<{ payload: ImposterTreeNode }>;
};

// Custom Content for TreeMap Node
const CustomizeContent = (props: ImposterTreemapContentProps) => {
    const { x = 0, y = 0, width = 0, height = 0, payload, name, value, net_value } = props;
...
const CustomTooltip = ({ active, payload }: ImposterTreemapTooltipProps) => {
    if (active && payload && payload.length) {
        ...
        return (
            <div className="bg-slate-900 border border-slate-700 p-3 rounded shadow-xl text-sm z-50">
                ...
                <div className="text-slate-300">Total Value: <span className="text-teal-400 font-bold">{formatRupiah(data.value ?? 0)}</span></div>
...
export function ImposterTreeMap({ data }: ImposterTreeMapProps) {
